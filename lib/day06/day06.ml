...
let%test _ =
    calc_ways_to_win 7 9 |> List.length = 4

let%test _ =
    calc_ways_to_win 15 40 |> List.length = 8

let process_races races =
    List.fold (List.map races ~f:(fun (time, dist) ->
        calc_ways_to_win time dist |> List.length
    )) ~init:1 ~f:( * )

let%test _ =
    process_races input = 288

let input_2 = [ (71530, 940200) ]

let%test _ = process_races input_2 = 71503
